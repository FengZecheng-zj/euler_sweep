...
int main(int argc, char** argv)
{
    ...
    Cube();
    listId1 = show_face(100);
    //DiySweeping();
    //listId2 = show_face(-100);
    House();
    listId3 = show_face(-100);    
...
void initGL()
{
    ...
    glClearColor(30.0 / 255, 75.0 / 255, 200.0/255,1);                   // background color
...
void showInfo()
{
    ...
    ss << "Example of 3D CAD Modelling" << ends;
    drawString(ss.str().c_str(), 1, 30, color, font);
    ss.str("");

    ss << "House(Euler and Sweep)" << ends;
    drawString(ss.str().c_str(), 100, 60, color, font);
    ss.str("");

    //ss << "Something(Euler and Sweep)" << ends;
    //drawString(ss.str().c_str(), 150, 60, color, font);
    //ss.str("");

    ss << "Cube(Pure Euler)" << ends;
    drawString(ss.str().c_str(), 200, 60, color, font);
    ss.str("");
